\documentclass[journal, retainorgcmds]{../IEEEtemplate/IEEEtran}
\usepackage{cite}
...
be more precise.
Wether we return a red-black or an infrared tree and wether we should call
...
The \iagda{insert} function has a more precise type but it is not as precise
as possible, i.e. the resulting \iagda{Tree} cannot have just any \iagda{Height}
but it is not uniquely defined either.
Returning a value as part of a disjoint sum here is a concession, the problem
with being more precise is that it is difficult to say in advance wether a
...
Haskell as a language is certainly not dependently typed but GHC \cite{ghc}
has several extensions that try to retrofit similar features into the
...
Agda's type system is not polymorphic and this problem was solved in a
different way through Agda's module system but that would take us too far.
DataKinds
GADTs/KindSignature
